...
RgbImage::RgbImage( int numRows, int numCols )
{
	...
	int rowLen = (GLint)GetNumBytesPerRow();
...
{  
	...
	for ( int i=0; i<NumRows; i++ ) {
		...
		GLint k=3*(GLint)NumCols;					// Num bytes already read
...
bool RgbImage::WriteBmpFile( const char* filename )
{
	...
	fputc('B',outfile);
	fputc('M',outfile);
	GLint rowLen = (GLint) GetNumBytesPerRow();
...
bool RgbImage::LoadFromOpenglBuffer()					// Load the bitmap from the current OpenGL buffer
{
	...
	if ( vWidth>=NumCols ) {
		...
		glPixelStorei( GL_UNPACK_ROW_LENGTH, (GLint)NumCols );
	}
	glPixelStorei(GL_UNPACK_ALIGNMENT, 4);

	// Get the frame buffer data.
	glReadPixels( 0, 0, (GLint)NumCols, (GLint)NumRows, GL_RGB, GL_UNSIGNED_BYTE, ImagePtr);
